...
bool initilizeGPU() {
	...
	for (int i = 0; i < TOTAL_TIME_STEPS; i++) {
		gpu_data->new_vehicles_every_time_step[i] = new NewLaneVehicles();
	}
...
bool initGPUData(GPUMemory* data_local) {
	...
	for (unsigned int i = 0; i < the_network->all_links.size(); i++) {
...
//		assert(1 == 0);
	...
	for (unsigned int i = 0; i < the_network->all_nodes.size(); i++) {
		...
		for (unsigned int j = 0; j < one_node->upstream_links.size(); j++) {
		...
		for (unsigned int j = 0; j < MAX_LANE_DOWNSTREAM; j++) {
			data_local->node_pool.downstream[j][i] = -1;
		}

		for (unsigned int j = 0; j < one_node->downstream_links.size(); j++) {
...
//		std::cout << "MAXIMUM_ACCUMULATED_FLOW:" << i << ", " << data_local->node_pool.MAXIMUM_ACCUMULATED_FLOW[i] << std::endl;
	...
	//Insert Vehicles
	for (unsigned int i = 0; i < all_vehicles.size(); i++) {
...
//		assert(time_index == time_index_covert);
		...
		if (time_index >= TOTAL_TIME_STEPS) continue;

		if (data_local->new_vehicles_every_time_step[time_index]->new_vehicle_size[lane_ID] < VEHICLE_MAX_LOADING_ONE_TIME) {
			int index = data_local->new_vehicles_every_time_step[time_index]->new_vehicle_size[lane_ID];

			data_local->new_vehicles_every_time_step[time_index]->new_vehicles[index][lane_ID].vehicle_ID = one_vehicle->vehicle_id;
			...
			for (int p = 1; p < max_copy_length; p++) {
				data_local->new_vehicles_every_time_step[time_index]->new_vehicles[index][lane_ID].path_code[p - 1] = all_od_paths[one_vehicle->path_id]->route_code[p] ? 1 : 0;
			}

//			for (int p = 1; p < max_copy_length; p++)
//				std::cout << "data_local->new_vehicles_every_time_step[time_index].new_vehicles[index][lane_ID].path_code:"
//						<< data_local->new_vehicles_every_time_step[time_index].new_vehicles[index][lane_ID].path_code[p-1] << ",";
			...
			data_local->new_vehicles_every_time_step[time_index]->new_vehicles[index][lane_ID].next_path_index = 1;
			...
			std::cout << "Loading Vehicles Exceeds The Loading Capacity: Time:" << time_index << ", Lane_ID:" << lane_ID << std::endl;
		}
	}
