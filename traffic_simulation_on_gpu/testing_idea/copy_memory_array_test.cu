...
//class GPU_Memory {
//public:
//	VehicleGroup* open_space;
//
//	VehicleGroup* lanes_groups[ROADS_SIZE];
//	int simulated_result;
//};
//
//__global__ void simulate_on_GPU(GPU_Memory* data_on_gpu, VehicleGroup* the_open_space_on_gpu) {
//	int idx = blockIdx.x * blockDim.x + threadIdx.x;
//
////	if (idx == 1 || idx == 0) {
//
//		for (int i = 0; i < ROADS_SIZE; i++) {
//			data_on_gpu->lanes_groups[i] = &(the_open_space_on_gpu[i]);
//		}
////
//		data_on_gpu->simulated_result = data_on_gpu->lanes_groups[0]->all_vehicles_on_lane[4].x;
//
//		printf("XXXXX:%d,\n", data_on_gpu->simulated_result);
////	}
//}
//
...
//using namespace std;
//
////VehicleGroup* the_open_space;
//VehicleGroup* the_open_space_on_gpu;
//
//void init_gpu() {
//	data_on_gpu = NULL;
//
...
//	data_on_cpu->open_space = (VehicleGroup*) malloc(sizeof(VehicleGroup) * ROADS_SIZE);
//
//	cout << "start 1:" << std::endl;
//
//	for (int i = 0; i < ROADS_SIZE; i++) {
//		data_on_cpu->lanes_groups[i] = &(data_on_cpu->open_space[i]);
//
////		cout << "i:" << i << std::endl;
//
...
//
//	if (cudaMalloc(&data_on_gpu, sizeof(VehicleGroup) * ROADS_SIZE + sizeof(VehicleGroup*) * ROADS_SIZE + sizeof(int)) != cudaSuccess) {
...
//	cudaMemcpy(the_open_space_on_gpu, data_on_cpu->open_space, sizeof(VehicleGroup) * ROADS_SIZE, cudaMemcpyHostToDevice);
//
//	cout << "start 3:" << std::endl;
//
//	simulate_on_GPU<<<1, 1>>>(data_on_gpu, the_open_space_on_gpu);
//
