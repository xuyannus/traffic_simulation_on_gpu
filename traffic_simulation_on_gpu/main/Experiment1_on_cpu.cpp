...
////std::string demand_file_path = "data/exp2/demand_100_1.dat";
...
//std::string simulation_output_file_path = "output/test_2_true.txt";
//
...
//
///*
// * Link ID and Link Index Convert
// */
...
// */
//GPUVehicle* get_next_vehicle_at_node_serial(int node_index, int* lane_id);
...
////		int node_id_in_GPU = one_node->node_id;
//
...
//				//path code is the link index in GPU memory
...
////				<< ":" << gpu_data->lane_pool.speed[lane_Index] << ":" << gpu_data->lane_pool.queue_length[lane_Index] << ")" << endl;
...
//
////		std::cout << "gpu_data->lane_pool.speed[lane_index]:" << gpu_data->lane_pool.speed[lane_index] << std::endl;
//
////Speed-Density Relationship
////		gpu_data->lane_pool.speed[lane_index] =
//
...
//
////			gpu_data->lane_pool.speed[lane_index] = gpu_data->lane_pool.MAX_SPEED[lane_index]
////					* (pow((1 - pow((gpu_data->lane_pool.density[lane_index] / gpu_data->lane_pool.max_density[lane_index]), gpu_data->lane_pool.beta[lane_index])),
////							gpu_data->lane_pool.alpha[lane_index]));
//		}
//
...
//		 */
//		if (EXECUTION_MODE == 1) {
//			gpu_data->lane_pool.predicted_empty_space[lane_index] = gpu_data->lane_pool.empty_space[lane_index];
////			gpu_data->lane_pool.debug_data[lane_index] = gpu_data->lane_pool.predicted_empty_space[lane_index];
//		}
//		else if (EXECUTION_MODE == 2) {
//			gpu_data->lane_pool.predicted_empty_space[lane_index] = gpu_data->lane_pool.last_time_empty_space[lane_index];
////			gpu_data->lane_pool.debug_data[lane_index] = gpu_data->lane_pool.predicted_empty_space[lane_index];
//		}
//		else if (EXECUTION_MODE == 3) {
//			//assume that there is no queue feedback
//			gpu_data->lane_pool.predicted_empty_space[lane_index] = ROAD_LENGTH;
////					std::min(gpu_data->lane_pool.last_time_empty_space[lane_index] + (gpu_data->lane_pool.speed[lane_index] * UNIT_TIME_STEPS),
////					1.0f * ROAD_LENGTH);
//
//		}
...
//
//				//
////				std::cout << "predicted_empty_space:" << gpu_data->lane_pool.predicted_empty_space[lane_index] << ",empty_space:" << gpu_data->lane_pool.empty_space[lane_index] << std::endl;
//			}
//
////			gpu_data->lane_pool.debug_data[lane_index] = gpu_data->lane_pool.predicted_empty_space[lane_index];
//		}
...
//		gpu_data->lane_pool.debug_data[lane_index] = gpu_data->lane_pool.predicted_empty_space[lane_index];
//
////		std::cout << "gpu_data->lane_pool.accumulated_offset[lane_index]:" << gpu_data->lane_pool.accumulated_offset[lane_index] << std::endl;
//
////update Tp
...
// */
//GPUVehicle* get_next_vehicle_at_node_serial(int node_index, int* lane_index) {
//
...
//					 */
//
//					int one_lane_id = link_Index_to_link_ID[one_lane_index];
//					int next_lane_id = link_Index_to_link_ID[next_lane_index];
//
...
//						if (gpu_data->lane_pool.input_capacity[next_lane_index] > 0 && gpu_data->lane_pool.empty_space[next_lane_index] >= VEHICLE_LENGTH) {
//							if (time_diff > maximum_waiting_time) {
//								maximum_waiting_time = time_diff;
//								*lane_index = one_lane_index;
//								the_one_veh = gpu_data->lane_pool.vehicle_space[0][one_lane_index];
...
////						std::cout << "good to be here; EXECUTION_MODE:" << EXECUTION_MODE << ",countAA:" << (countAA++) << std::endl;
//
//						/**
...
//
////							if (gpu_data->lane_pool.input_capacity[next_lane_index] <= 0) {
////								static int countDD = 0;
////								std::cout << "good to be here; EXECUTION_MODE:" << EXECUTION_MODE << ",countDD:" << (countDD++) << std::endl;
////							}
//
////							else if (gpu_data->lane_pool.predicted_empty_space[next_lane_index] < VEHICLE_LENGTH) {
...
////								static int countFF = 0;
////								std::cout << "good to be here; EXECUTION_MODE:" << EXECUTION_MODE << ",countFF:" << (countFF++) << std::endl;
//
//								if (time_diff > maximum_waiting_time) {
//									maximum_waiting_time = time_diff;
//									*lane_index = one_lane_index;
...
////								std::cout << "good to be here; EXECUTION_MODE:" << EXECUTION_MODE << ",countEE:" << (countEE++) << std::endl;
//								gpu_data->lane_pool.blocked[one_lane_index] = true;
//							}
//						}
//						else {
...
//
//			//Find A vehicle
//			GPUVehicle* one_v = get_next_vehicle_at_node_serial(node_index, &lane_index);
//
...
//
//		//update queue status
//		for (unsigned int temp_index = 0; temp_index < the_network->all_nodes[node_index]->upstream_links.size(); temp_index++) {
//			unsigned int lane_id = the_network->all_nodes[node_index]->upstream_links[temp_index]->link_id;
//			int lane_index = link_ID_to_link_Index[lane_id];
//
//			//update queue length
...
//						acc_length_moving -= gpu_data->lane_pool.speed_history[j][lane_index] * UNIT_TIME_STEPS;
//					}
//
...
//			}
//			gpu_data->lane_pool.his_queue_length[0][lane_index] = queue_length;
//
