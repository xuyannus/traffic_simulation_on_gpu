...
//init capacity
#ifdef ENABLE_CONSTANT_MEMORY
	gpu_data->lane_pool.input_capacity[lane_index] = data_setting_gpu_constant.ON_GPU_LANE_INPUT_CAPACITY_TIME_STEP;
	gpu_data->lane_pool.output_capacity[lane_index] = data_setting_gpu_constant.ON_GPU_LANE_OUTPUT_CAPACITY_TIME_STEP;
#else
	gpu_data->lane_pool.input_capacity[lane_index] = data_setting_gpu->ON_GPU_LANE_INPUT_CAPACITY_TIME_STEP;
	gpu_data->lane_pool.output_capacity[lane_index] = data_setting_gpu->ON_GPU_LANE_OUTPUT_CAPACITY_TIME_STEP;
#endif
...
//				gpu_data->lane_pool.new_vehicle_join_counts[lane_index]++;
	...
	if (gpu_data->lane_pool.vehicle_passed_to_the_lane_counts[lane_index] > 0) {
		...
		gpu_data->lane_pool.empty_space[lane_index] = min_device(gpu_data->lane_pool.speed[lane_index] * data_setting_gpu_constant.ON_GPU_UNIT_TIME_STEPS, gpu_data->lane_pool.empty_space[lane_index])
		- gpu_data->lane_pool.vehicle_passed_to_the_lane_counts[lane_index] * data_setting_gpu_constant.ON_GPU_VEHICLE_LENGTH;
#else
		gpu_data->lane_pool.empty_space[lane_index] = min_device(gpu_data->lane_pool.speed[lane_index] * data_setting_gpu->ON_GPU_UNIT_TIME_STEPS, gpu_data->lane_pool.empty_space[lane_index])
				- gpu_data->lane_pool.vehicle_passed_to_the_lane_counts[lane_index] * data_setting_gpu->ON_GPU_VEHICLE_LENGTH;
#endif
...
#ifdef ENABLE_CONSTANT_MEMORY
	density_ = 1.0 * data_setting_gpu_constant.ON_GPU_VEHICLE_LENGTH * gpu_data->lane_pool.vehicle_counts[lane_index] / data_setting_gpu_constant.ON_GPU_ROAD_LENGTH;
#else
	density_ = 1.0 * data_setting_gpu->ON_GPU_VEHICLE_LENGTH * gpu_data->lane_pool.vehicle_counts[lane_index] / gpu_data->lane_pool.lane_length[lane_index];
#endif
...
#ifdef ENABLE_CONSTANT_MEMORY
		gpu_data->lane_pool.predicted_empty_space[lane_index] = min_device(gpu_data->lane_pool.last_time_empty_space[lane_index] + (speed_ * data_setting_gpu_constant.ON_GPU_UNIT_TIME_STEPS),
...
//				* gpu_data->lane_pool.his_queue_length_weighting[2][lane_index];
		...
		gpu_data->lane_pool.predicted_empty_space[lane_index] = min_device(
				...
				(data_setting_gpu->ON_GPU_ROAD_LENGTH - prediction_queue_length_));
#endif
	}
...
//update Tp
...
#ifdef ENABLE_CONSTANT_MEMORY
	gpu_data->lane_pool.accumulated_offset[lane_index] += speed_ * data_setting_gpu_constant.ON_GPU_UNIT_TIME_STEPS; //meter

	while (gpu_data->lane_pool.accumulated_offset[lane_index] >= data_setting_gpu_constant.ON_GPU_ROAD_LENGTH) {
		gpu_data->lane_pool.accumulated_offset[lane_index] -= gpu_data->lane_pool.speed_history[gpu_data->lane_pool.Tp[lane_index]][lane_index] * data_setting_gpu_constant.ON_GPU_UNIT_TIME_STEPS;
		gpu_data->lane_pool.Tp[lane_index] += data_setting_gpu_constant.ON_GPU_UNIT_TIME_STEPS;
	}
#else

	gpu_data->lane_pool.accumulated_offset[lane_index] += gpu_data->lane_pool.speed[lane_index] * data_setting_gpu->ON_GPU_UNIT_TIME_STEPS; //meter

	while (gpu_data->lane_pool.accumulated_offset[lane_index] >= gpu_data->lane_pool.lane_length[lane_index]) {
		gpu_data->lane_pool.accumulated_offset[lane_index] -= gpu_data->lane_pool.speed_history[gpu_data->lane_pool.Tp[lane_index]][lane_index] * data_setting_gpu->ON_GPU_UNIT_TIME_STEPS;
		gpu_data->lane_pool.Tp[lane_index] += data_setting_gpu->ON_GPU_UNIT_TIME_STEPS;
	}
#endif

	//update queue length
	int queue_start = gpu_data->lane_pool.queue_length[lane_index] / data_setting_gpu->ON_GPU_VEHICLE_LENGTH;
	for (int queue_index = queue_start; queue_index < gpu_data->lane_pool.vehicle_counts[lane_index]; queue_index++) {
		if (gpu_data->lane_pool.vehicle_space[queue_index][lane_index]->entry_time <= gpu_data->lane_pool.Tp[lane_index]) {
			gpu_data->lane_pool.queue_length[lane_index] += data_setting_gpu->ON_GPU_VEHICLE_LENGTH;
		}
		else {
			break;
		}
...
//update speed and density
#ifdef ENABLE_CONSTANT_MEMORY
	gpu_data->lane_pool.density[lane_index] = 1.0 * data_setting_gpu_constant.ON_GPU_VEHICLE_LENGTH * gpu_data->lane_pool.vehicle_counts[lane_index] / gpu_data->lane_pool.lane_length[lane_index];
#else
	gpu_data->lane_pool.density[lane_index] = 1.0 * data_setting_gpu->ON_GPU_VEHICLE_LENGTH * gpu_data->lane_pool.vehicle_counts[lane_index] / gpu_data->lane_pool.lane_length[lane_index];
#endif
...
//		gpu_data->lane_pool.predicted_empty_space[lane_index] = gpu_data->lane_pool.his_queue_length[0][lane_index];
...
#ifdef ENABLE_CONSTANT_MEMORY
		gpu_data->lane_pool.predicted_empty_space[lane_index] = min_device(
				gpu_data->lane_pool.last_time_empty_space[lane_index] + (gpu_data->lane_pool.speed[lane_index] * data_setting_gpu_constant.ON_GPU_UNIT_TIME_STEPS),
				1.0f * data_setting_gpu_constant.ON_GPU_ROAD_LENGTH);
#else
		gpu_data->lane_pool.predicted_empty_space[lane_index] = min_device(
				gpu_data->lane_pool.last_time_empty_space[lane_index] + (gpu_data->lane_pool.speed[lane_index] * data_setting_gpu->ON_GPU_UNIT_TIME_STEPS), 1.0f * data_setting_gpu->ON_GPU_ROAD_LENGTH);
		...
		* gpu_data->lane_pool.his_queue_length_weighting[2][lane_index];
		...
		gpu_data->lane_pool.predicted_empty_space[lane_index] = min_device(
				gpu_data->lane_pool.last_time_empty_space[lane_index] + (gpu_data->lane_pool.speed[lane_index] * data_setting_gpu_constant.ON_GPU_UNIT_TIME_STEPS),
				(data_setting_gpu_constant.ON_GPU_ROAD_LENGTH - gpu_data->lane_pool.predicted_queue_length[lane_index]));
#else
		gpu_data->lane_pool.predicted_empty_space[lane_index] = min_device(
				gpu_data->lane_pool.last_time_empty_space[lane_index] + (gpu_data->lane_pool.speed[lane_index] * data_setting_gpu->ON_GPU_UNIT_TIME_STEPS),
				(data_setting_gpu->ON_GPU_ROAD_LENGTH - gpu_data->lane_pool.predicted_queue_length[lane_index]));
#endif
	}
...
#ifdef ENABLE_CONSTANT_MEMORY
	gpu_data->lane_pool.accumulated_offset[lane_index] += gpu_data->lane_pool.speed[lane_index] * data_setting_gpu_constant.ON_GPU_UNIT_TIME_STEPS; //meter
...
//						return gpu_data->lane_pool.vehicle_space[0][one_lane_index];
					 ...
					 */
#ifdef ENABLE_CONSTANT_MEMORY
					if (gpu_data->lane_pool.input_capacity[next_lane_index] > 0 && gpu_data->lane_pool.predicted_empty_space[next_lane_index] > data_setting_gpu_constant.ON_GPU_VEHICLE_LENGTH) {
#else
					if (gpu_data->lane_pool.input_capacity[next_lane_index] > 0 && gpu_data->lane_pool.predicted_empty_space[next_lane_index] > data_setting_gpu->ON_GPU_VEHICLE_LENGTH) {
#endif
						if (time_diff > maximum_waiting_time) {
							maximum_waiting_time = time_diff;
							*lane_index = one_lane_index;
...
#ifdef ENABLE_CONSTANT_MEMORY
			gpu_data->lane_pool.predicted_empty_space[next_lane_index] -= data_setting_gpu_constant.ON_GPU_VEHICLE_LENGTH;
#else
			gpu_data->lane_pool.predicted_empty_space[next_lane_index] -= data_setting_gpu->ON_GPU_VEHICLE_LENGTH;
#endif
...
//				acc_length_moving -= gpu_data->lane_pool.speed_history[j][lane_index] * data_setting_gpu->ON_GPU_UNIT_TIME_STEPS;
//			}
//
...
//			}
//			else {
	gpu_data->lane_pool.empty_space[lane_index] = gpu_data->lane_pool.empty_space[lane_index] + gpu_data->lane_pool.speed[lane_index] * data_setting_gpu->ON_GPU_UNIT_TIME_STEPS;
	gpu_data->lane_pool.empty_space[lane_index] = min_device(data_setting_gpu->ON_GPU_ROAD_LENGTH - gpu_data->lane_pool.queue_length[lane_index], gpu_data->lane_pool.empty_space[lane_index]);
